...
// 앞에 숫자가 더 붙은 "1223시"는 끝 두 자리만 떼어 23시로 읽지 않도록 (?<!\d)로 막는다.
const HOUR_PATTERN = /(오전|오후)?\s*(?<!\d)(\d{1,2})\s*시(?!간)(?:\s*(반|(\d{1,2})\s*분))?/;
// "19:30", "오후 7:30" — "123:45"도 같은 이유로 제외
const COLON_PATTERN = /(오전|오후)?\s*(?<!\d)(\d{1,2}):(\d{2})/;
